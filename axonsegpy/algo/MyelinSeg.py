...
def segMyelin(axon,image,labeledMask,deltaVecs,snake = False, verbose = False,maxMyelinWidth = 15,diffDegree = 1, outlierDifinition = 1, nbIterations = 5):
    ...
    for i in range(nbIterations):
        ...
        std = np.std(sizes) * outlierDifinition
...
def run(params):
    ...
    if "display" in params:
        display = params["display"] is "full"
    else:
        display = False
    if display:
        myelinVisual = MyelinVisualisationSuperDuperFancy(testImage,melynList)
    else:
